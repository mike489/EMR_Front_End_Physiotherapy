...
import AddThoracic from './components/AddThoracic';

const ThoracicTab = ({ visit }) => {
  ...
  const handleFetchingComplaints = useCallback(async () => {
    ...
    const Api = `${Backend.auth}${Backend.complaints}?page=${pagination.page + 1}&per_page=${pagination.per_page}&search=${search}&visit_id=${visit?.visit_id}`;
    const header = {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {loading ? (
        ...
        <ErrorPrompt
          title="Server Error"
          message="Unable to retrieve Thoracic."
        />
      ) : complaints.length === 0 ? (
        <Fallbacks
          severity="info"
          title="No Thoracic data Found"
          description="Thoracic data will be listed here when available."
          sx={{ paddingTop: 6 }}
        />
      ) : (
