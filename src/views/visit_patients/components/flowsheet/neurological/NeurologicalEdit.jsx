...
const NeurologicalEdit = ({ assessment, onClose }) => {
  ...
  useEffect(() => {
    if (assessment) {
      setFormData({
        ...
        spinal_arrangement: assessment.spinal_arrangement || '',
  ...
  const handleUpdate = async () => {
    ...
    const Api = `${Backend.auth}${Backend.neurologicalAssessments}/${assessment.id}`;
    const headers = {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Neurological assessment updated successfully!');
        setErrors({
          assessment_date: '',
          mental_status: '',
          extremities: '',
...
export default NeurologicalEdit;