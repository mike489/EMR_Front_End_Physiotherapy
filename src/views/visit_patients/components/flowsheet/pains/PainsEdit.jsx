...
const PainsEdit = ({ assessment, onClose }) => {
  ...
  useEffect(() => {
    console.log('Assessment prop:', assessment); // Debugging
    if (assessment) {
      setFormData({
        pain_score: assessment.pain_score || '',
        pain_location: assessment.pain_location || '',
        pain_assessment: assessment.pain_assessment || '',
        pain_type: assessment.pain_type || '',
        orientation: assessment.orientation || '',
        pain_onset: assessment.pain_onset || '',
        pain_frequency: assessment.pain_frequency || '',
        clinical_progress: assessment.clinical_progress || '',
      });
    }
  }, [assessment]);
  ...
  const validateForm = () => {
    ...
    if (!formData.pain_frequency) {
      ...
      valid = false;
    } else if (!painFrequencyOptions.includes(formData.pain_frequency)) {
      newErrors.pain_frequency = 'Invalid pain frequency selected';
      valid = false;
    }
  ...
  const handleUpdate = async () => {
    ...
    const Api = `${Backend.auth}${Backend.pains}/${assessment.id}`;
    const payload = {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.data?.message || 'Failed to update pain record');
      }
    } catch (error) {
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          ...
          <ReactQuill
            ...
            style={{
              minHeight: '150px',
              border: errors.pain_assessment ? '1px red' : '1px #ccc',
              borderRadius: '4px',
            }}
          />
          <Typography
        ...
        <Grid item xs={12}>
          ...
          <ReactQuill
            ...
            style={{
              minHeight: '150px',
              border: errors.clinical_progress ? '1px red' : '1px #ccc',
              borderRadius: '4px',
            }}
          />
          <Typography
        ...
        <Grid item xs={12} sx={{ mt: 2 }}>
          ...
          >
            {submitting ? <CircularProgress size={24} /> : 'Update Pain Record'}
          </Button>
          {onClose && (
            <Button
              variant="outlined"
