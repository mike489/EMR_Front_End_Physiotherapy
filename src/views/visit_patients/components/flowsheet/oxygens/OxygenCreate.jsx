...
const OxygenCreate = ({ visit, onClose }) => {
  ...
  const handleCreate = async () => {
    ...
    const Api = `${Backend.auth}${Backend.oxygens}`;
    const headers = {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Oxygen record created successfully');
        setFormData({
          spo2_level: '',
          spo2_measurement: '',
          pulse_oximetry_type: '',
        ...
        toast.error(data.data?.message || 'Failed to create record');
      }
    } catch (err) {
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sx={{ mt: 2 }}>
          ...
          >
            {submitting ? <CircularProgress size={24} /> : 'Create Oxygen Assessment'}
          </Button>
          )}
          {onClose && (
            <Button
