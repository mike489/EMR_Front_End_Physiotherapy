...
const EditVitals = ({ visit, vital, onClose }) => {
  ...
  const [vitalData, setVitalData] = useState({
    ...
    bp_method: vital?.bp_method || '',
    bp_location: vital?.bp_location || '',
  });
  const [errors, setErrors] = useState({});

  // Options for bp_method and bp_location (same as CreateVitals)
  ...
  const handleUpdateVital = async () => {
    ...
    const Api = `${Backend.auth}${Backend.vitalSigns}/${vitalData.id}`;
    const header = {
    ...
    try {
      ...
      if (response.ok && responseData.success) {
        ...
        toast.error(responseData.data.message || 'Failed to update vital');
      }
    } catch (error) {
      toast.error('Error updating vital: ' + error.message);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sx={{ mt: 2 }}>
          ...
          >
            {submitting ? <CircularProgress size={24} /> : 'Update Vital'}
          </Button>
          )}
          {onClose && (
            <Button
