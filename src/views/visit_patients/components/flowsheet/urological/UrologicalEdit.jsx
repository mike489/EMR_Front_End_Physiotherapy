...
const UrologicalEdit = ({ assessment, onClose }) => {
  ...
  useEffect(() => {
    if (assessment) {
      setFormData({
        ...
        urinary_amount: assessment.urinary_amount || '',
        urinary_color: assessment.urinary_color || '',
        electrolyte_status: assessment.electrolyte_status || '',
        bacteria_status: assessment.bacteria_status || '',
        dialysis: assessment.dialysis || '',
      });
    }
  }, [assessment]);
  ...
  const handleUpdate = async () => {
    ...
    const Api = `${Backend.auth}${Backend.urologicalAssessments}/${assessment.id}`;
    const headers = {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Urological assessment updated successfully!');
        setErrors({});
        if (onClose) onClose();
