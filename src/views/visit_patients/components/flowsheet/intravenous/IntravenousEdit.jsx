...
const IntravenousEdit = ({ assessment, onClose }) => {
  ...
  useEffect(() => {
    if (assessment) {
      setFormData({
        ...
        site_inspection: assessment.site_inspection || '',
        infiltration: assessment.infiltration || '',
        tubing: assessment.tubing || '',
        rate: assessment.rate || '',
        fluid_overload: assessment.fluid_overload || '',
      });
    }
  }, [assessment]);
  ...
  const handleUpdate = async () => {
    ...
    const Api = `${Backend.auth}${Backend.intravenousTherapies}/${assessment.id}`;
    const headers = {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Intravenous therapy assessment updated successfully!');
        setErrors({
          date: '',
          site_inspection: '',
          infiltration: '',
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: 'bold' }}>
        Edit Intravenous Therapy Assessment
      </Typography>
...
export default IntravenousEdit;