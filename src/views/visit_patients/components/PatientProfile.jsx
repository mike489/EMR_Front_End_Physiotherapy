...
import { Person, Fingerprint, Email, Phone, Transgender, Cake, CreditCard, Home, Category, Warning, History, MedicalServices } from '@mui/icons-material';
        ...
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={10}>
                <Grid container spacing={2}>
                    ...
                    {renderDetail(Cake, 'Date of Birth', visit?.date_of_birth && new Date(visit?.date_of_birth).toLocaleDateString())}
                    {renderDetail(CreditCard, 'Payment', visit?.payment_type)}
                    {renderDetail(Home, 'Address', visit?.address && `${visit?.address?.wereda}, ${visit?.address?.city}`)}
                    {renderDetail(Warning, 'Allergies', visit?.allergy)}
                    {renderDetail(History, 'Medical History', visit?.medical_history)}
                    {renderDetail(MedicalServices, 'Medical Conditions', visit?.medical_conditions)}
                </Grid>
            </Grid>
        </Grid>
    );
