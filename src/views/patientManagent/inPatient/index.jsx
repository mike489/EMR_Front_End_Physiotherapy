...
const InPatients = () => {
  ...
  const handleFetchingPatients = async () => {
    ...
    const Api = `${Backend.auth}${Backend.patientsInPatients}?${params.toString()}`;
    const header = {
  ...
  return (
    <PageContainer title="In Patients">
      <Grid container>
          ...
          <Grid container>
            <Grid item xs={12}>
              {loading ? (
                ...
                <ErrorPrompt
                  title="Server Error"
                  message="Unable to retrieve in patients."
                />
              ) : data.length === 0 ? (
                <Fallbacks
                  severity="evaluation"
                  title="In Patients Not Found"
                  description="The list of in patients will be listed here."
                  sx={{ paddingTop: 6 }}
                />
              ) : (
                <TableContainer
