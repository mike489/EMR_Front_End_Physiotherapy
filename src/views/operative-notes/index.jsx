...
export default function NotesIndex() {
 ...
 const fetchPatients = async () => {
  ...
  const fetchNotes = async (page = 1, perPage = 10, type = "") => {
    const token = await GetToken();
    let Api = `${Backend.auth}${Backend.notes}?page=${page}&per_page=${perPage}`;
  ...
  const handleAddNote = async () => {
    ...
    if (!formData.type) {
      toast.warning("Please select a note type");
      return;
    }
    ...
    const token = await GetToken();
    const Api = `${Backend.auth}${Backend.notes}`;
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({ patient_id: "", content: "", type: "" });
        setOpenForm(false);
