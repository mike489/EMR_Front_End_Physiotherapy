...
const ScheduleDoctor = () => {
  ...
  const findExistingMatch = () => {
    ...
    if (period !== 'Custom') {
      return existing.find((row) => {
        if ((row.period || '').toLowerCase() !== period.toLowerCase()) return false;
    ...
    const norm = (arr) => (arr || []).map((d) => String(d).toLowerCase()).sort().join(',');
  ...
  const validate = () => {
    ...
    if (period === 'Custom') {
      const validDates = (customDates || []).map((d) => d && toSqlDate(d)).filter((d) => !!d);
      if (validDates.length === 0) return 'Select at least one date for custom period';
    }
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (period === 'Custom') {
      const validDates = (customDates || []).map((d) => d && toSqlDate(d)).filter((d) => !!d);
      const customPayload = validDates.map((dateStr) => ({ date: dateStr, time_slots: timeSlots }));
      const body = {
        doctor_id: doctor.id,
        period: 'Custom',
        custom_dates: customPayload,
      };
      const res = await fetch(Api, { method: 'POST', headers: header, body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok || data?.success === false) {
        const msg = data?.data?.message || data?.message || 'Failed to create availability';
        throw new Error(msg);
      }
    } else {
      const body = {
    ...
    <PageContainer back={true} title="Schedule Doctor">
      <Card sx={{ p: 2, mb: 4, borderRadius: 3 }}>
        ...
        <Grid container spacing={2}>
          ...
          {period === 'Custom' && (
            <Grid item xs={12}>
              <Typography variant="subtitle1" sx={{ mb: 1 }}>Custom Dates</Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {customDates.map((d, idx) => (
                  <Box key={`date-${idx}`} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <TextField
                      type="date"
                      label={`Date ${idx + 1}`}
                      size="small"
                      value={d}
                      onChange={(e) => setCustomDates((arr) => arr.map((v, i) => (i === idx ? e.target.value : v)))}
                      InputLabelProps={{ shrink: true }}
                    />
                    <IconButton color="error" onClick={() => setCustomDates((arr) => arr.filter((_, i) => i !== idx))} disabled={customDates.length === 1}>
                      <Remove />
                    </IconButton>
                    <IconButton color="primary" onClick={() => setCustomDates((arr) => [...arr, ''])}>
                      <Add />
                    </IconButton>
                  </Box>
                ))}
          ...
          <Grid item xs={12}>
            <Typography variant="subtitle1" sx={{ mb: 1 }}>Time Slots</Typography>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {timeSlots.map((slot, idx) => (
                <Box key={`slot-${idx}`} sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
