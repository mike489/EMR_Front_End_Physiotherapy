...
const DoctorAvailabilityTab = ({ doctor }) => {
  ...
  const fetchDoctorAvailability = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(responseData.message || 'Failed to fetch availability');
  ...
  const handleOpenEdit = (row) => {
    ...
    if (isCustom) {
      if (row?.available_date) {
        ...
        const cds =
          Array.isArray(row?.custom_dates) && row.custom_dates.length > 0
            ? row.custom_dates.map((cd) => ({
                date: cd.date || cd.available_date || '',
                time_slots: (cd.time_slots || []).map((s) =>
                  typeof s === 'string'
                    ? {
  ...
  const validateSlots = () => {
    ...
    if (isCustom) {
      ...
      for (const cd of customDates) {
        ...
        if (!Array.isArray(cd.time_slots) || cd.time_slots.length === 0)
          return 'Each date needs at least one time slot';
      ...
      if (isCustom) {
        if (currentAvailability?.available_date) {
          ...
          body = {
            ...
            time_slots: (first.time_slots || []).map(
  ...
  return (
    <Box sx={{ mt: 2, padding: 2 }}>
              ...
              (() => {
                ...
                const groups = (availabilityRows || []).reduce((acc, row) => {
                  ...
                  if (row?.available_date && Array.isArray(row?.time_slots)) {
                    ...
                    Array.isArray(row?.custom_dates) &&
                    row.custom_dates.length > 0
                  ) {
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => handleOpenEdit(entry._source)}
                          >
                            <Edit fontSize="small" />
                          </IconButton>
                          <IconButton
                            size="small"
                            color="error"
                            onClick={() => handleDelete(entry._source)}
                          >
      ...
      <Dialog open={editOpen} onClose={handleCloseEdit} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          {String(currentAvailability?.period || '').toLowerCase() ===
          'custom' ? (
            <Box
                  ...
                  >
                    <TextField
                      ...
                      onChange={(e) =>
                        setCustomDates((arr) =>
                          arr.map((v, i) =>
                            i === cdIdx ? { ...v, date: e.target.value } : v,
                          ),
                        )
                      }
                  ...
                  {(cd.time_slots || []).map((slot, idx) => (
                    ...
                    >
                          ...
                          setCustomDates((arr) =>
                            arr.map((v, i) =>
                              i === cdIdx
                                ? {
                                    ...v,
                                    time_slots: v.time_slots.map((s, si) =>
                                      si === idx
                      ...
                      <IconButton
                          ...
                          setCustomDates((arr) =>
                            arr.map((v, i) =>
                              i === cdIdx
                                ? {
                                    ...v,
                                    time_slots: v.time_slots.filter(
                                      (_, si) => si !== idx,
                                    ),
                                  }
                                : v,
                        ...
                        disabled={(cd.time_slots || []).length === 1}
                      >
                        <Remove />
                      </IconButton>
                      <IconButton
